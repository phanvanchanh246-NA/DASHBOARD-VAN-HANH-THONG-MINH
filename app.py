...
# CẤU HÌNH
# ════════════════════════════════════════════════════════════════════
st.set_page_config(page_title="GHN · Báo cáo vận hành khu vực", page_icon="📑",
...
INK      = "#142033"   # tiêu đề, chữ chính — gần đen, ngả xanh
INK_2    = "#2A3A52"   # đường biểu đồ phụ, liên kết
PAPER    = "#F1F2ED"   # nền trang — giấy ngà lạnh, không phải kem ấm
CARD     = "#FFFFFF"
RULE     = "#DBDCD4"   # gạch mảnh
RULE_STR = "#B7B9AE"   # gạch đậm hơn cho dòng tổng
BRASS    = "#9C7A3C"   # đồng trầm — thay cho cam, dùng tiết kiệm
BRASS_DP = "#7A5D2C"
FOREST   = "#1F6E4A"   # số dương
BURGUNDY = "#8C2F39"   # số âm
SLATE    = "#5C6472"   # chữ phụ
...
pio.templates["report"] = go.layout.Template(layout=dict(
    font=dict(family="Public Sans, sans-serif", size=12, color=SLATE),
    plot_bgcolor=CARD, paper_bgcolor=CARD, hovermode="x unified",
    colorway=[INK, BRASS, FOREST, BURGUNDY, INK_2, SLATE],
    margin=dict(l=44, r=16, t=14, b=36),
    ...
    yaxis=dict(showgrid=True, gridcolor="#EAEBE6", gridwidth=1, zeroline=False,
               tickfont=dict(size=10.5, color=SLATE)),
    legend=dict(orientation="h", yanchor="bottom", y=1.0, xanchor="left", x=0,
                font=dict(size=10.5)),
    hoverlabel=dict(bgcolor=INK, bordercolor=INK, font=dict(family="Public Sans", size=12, color="#fff")),
))
pio.templates.default = "report"

st.markdown(f"""
<style>
@import url('https://fonts.googleapis.com/css2?family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,500;0,8..60,600;0,8..60,700;1,8..60,400;1,8..60,500&family=Public+Sans:wght@400;500;600;700&display=swap');
...
.letterhead {{
  ...
  padding-bottom:14px; margin-bottom:0; border-bottom:3px double var(--rules);
}}
.letterhead .lockup {{ display:flex; align-items:center; gap:14px; }}
.letterhead .lockup-text {{ font-family:'Source Serif 4',serif; font-weight:600; font-size:15px;
  color:var(--ink); text-transform:uppercase; letter-spacing:.06em; line-height:1.3; }}
.letterhead .lockup-text span {{ display:block; font-family:'Public Sans',sans-serif; font-weight:500;
  font-size:10.5px; letter-spacing:.12em; color:var(--slate); text-transform:uppercase; margin-top:2px; }}
.letterhead .period {{ text-align:right; font-size:11px; letter-spacing:.08em; color:var(--slate);
  text-transform:uppercase; line-height:1.7; }}
.letterhead .period b {{ color:var(--ink); font-weight:600; }}

/* ── mục header với số ma khổng lồ ────────────────────────────────── */
.note-head {{ position:relative; margin:34px 0 20px; padding-bottom:12px; border-bottom:2px solid var(--ink); overflow:hidden; }}
.note-head .ghost {{ position:absolute; right:-6px; top:-34px; font-family:'Source Serif 4',serif;
  font-weight:700; font-size:118px; color:var(--ink); opacity:.05; line-height:1; user-select:none; pointer-events:none; }}
.note-head .eyebrow {{ font-size:11px; letter-spacing:.14em; text-transform:uppercase; color:var(--brassd); font-weight:600; margin-bottom:3px; }}
.note-head h2 {{ font-size:26px; font-weight:600; color:var(--ink); margin:0; letter-spacing:.005em; }}
.note-head .sub {{ font-size:12.5px; color:var(--slate); margin-top:4px; }}

.sub-head {{ font-family:'Source Serif 4',serif; font-weight:600; font-size:16px; color:var(--ink);
  margin:26px 0 10px; padding-top:14px; border-top:1px solid var(--rule); }}

/* ── thư điều hành ────────────────────────────────────────────────── */
.letter {{ background:var(--card); border:1px solid var(--rule); padding:26px 30px 22px; margin:18px 0 6px; }}
.letter .kicker {{ font-size:10.5px; letter-spacing:.14em; text-transform:uppercase; color:var(--brassd); font-weight:600; margin-bottom:10px; }}
.letter p {{ font-family:'Source Serif 4',serif; font-style:italic; font-size:16.5px; line-height:1.75; color:var(--ink); margin:0; }}
.letter .dropcap {{ float:left; font-family:'Source Serif 4',serif; font-style:normal; font-weight:700;
  font-size:52px; line-height:.82; padding:4px 8px 0 0; color:var(--brassd); }}
.letter .sign {{ text-align:right; margin-top:14px; font-family:'Public Sans',sans-serif; font-style:normal;
  font-size:11px; letter-spacing:.06em; color:var(--slate); text-transform:uppercase; }}

/* ── số nổi bật ───────────────────────────────────────────────────── */
.hl-row {{ display:flex; background:var(--card); border:1px solid var(--rule); margin:18px 0; overflow-x:auto; }}
.hl {{ flex:1; min-width:150px; padding:18px 20px; border-right:1px solid var(--rule); }}
.hl:last-child {{ border-right:none; }}
.hl .cap {{ font-size:10px; letter-spacing:.1em; text-transform:uppercase; color:var(--slate); font-weight:600; margin-bottom:8px; }}
.hl .num {{ font-family:'Source Serif 4',serif; font-weight:600; font-size:27px; color:var(--ink);
  font-variant-numeric:tabular-nums; line-height:1; }}
.hl .num small {{ font-size:13px; font-weight:400; color:var(--slate); }}
.hl .delta {{ font-size:11.5px; margin-top:7px; font-variant-numeric:tabular-nums; font-weight:500; }}
.d-up {{ color:var(--forest); }} .d-down {{ color:var(--burg); }} .d-flat {{ color:var(--slate); }}

/* ── điểm cần lưu ý ───────────────────────────────────────────────── */
.notice {{ background:var(--card); border:1px solid var(--rule); border-top:3px solid var(--brass); padding:18px 22px; margin:18px 0; }}
.notice .cap {{ font-size:10.5px; letter-spacing:.14em; text-transform:uppercase; color:var(--brassd); font-weight:600; margin-bottom:12px; }}
.notice ol {{ margin:0; padding-left:20px; }}
.notice li {{ font-size:13.5px; line-height:1.75; color:var(--ink); margin-bottom:9px; }}
.notice li:last-child {{ margin-bottom:0; }}
.notice li b {{ font-variant-numeric:tabular-nums; font-weight:600; }}
.notice li .tag {{ font-size:10px; letter-spacing:.06em; text-transform:uppercase; padding:1px 6px;
  border-radius:2px; margin-right:7px; font-weight:600; }}
.tag-high {{ background:#FBEAEA; color:var(--burg); }}
.tag-mid  {{ background:#F5EFE1; color:var(--brassd); }}
.tag-ok   {{ background:#E9F3EC; color:var(--forest); }}

/* ── bảng sổ cái ──────────────────────────────────────────────────── */
table.ledger {{ width:100%; border-collapse:collapse; margin:14px 0 4px; font-size:13px; }}
table.ledger caption {{ text-align:left; font-size:10.5px; letter-spacing:.1em; text-transform:uppercase;
  color:var(--slate); font-weight:600; padding-bottom:8px; caption-side:top; }}
table.ledger th {{ text-align:right; font-weight:600; font-size:10.5px; letter-spacing:.06em;
  text-transform:uppercase; color:var(--slate); padding:0 0 8px; border-bottom:2px solid var(--ink); }}
table.ledger th:first-child, table.ledger td:first-child {{ text-align:left; }}
table.ledger td {{ text-align:right; padding:8px 0; border-bottom:1px solid var(--rule);
  font-variant-numeric:tabular-nums; color:var(--ink); }}
table.ledger tr.total td {{ border-top:2px solid var(--ink); border-bottom:3px double var(--ink);
  font-weight:600; padding-top:10px; padding-bottom:10px; }}
table.ledger td.up {{ color:var(--forest); }} table.ledger td.down {{ color:var(--burg); }}
table.ledger td.name {{ font-variant-numeric:normal; }}

/* ── caption cho biểu đồ ──────────────────────────────────────────── */
.fig-cap {{ font-size:10.5px; letter-spacing:.08em; text-transform:uppercase; color:var(--slate);
  font-weight:600; margin:16px 0 -6px; }}

.empty {{ background:var(--card); border:1px dashed var(--rule); padding:22px; text-align:center;
  color:var(--slate); font-size:13px; margin:14px 0; }}
...
.toc-label {{ font-size:10px; letter-spacing:.16em; text-transform:uppercase; color:var(--slate);
  font-weight:600; margin:2px 0 10px; }}
section[data-testid="stSidebar"] [role="radiogroup"] label {{
  font-family:'Public Sans',sans-serif !important; font-size:12.5px !important;
  letter-spacing:.01em; padding:7px 0; border-bottom:1px solid var(--rule); }}
section[data-testid="stSidebar"] [role="radiogroup"] label[data-checked="true"] {{ color:var(--brassd) !important; font-weight:600; }}

/* ── điều khiển Streamlit ─────────────────────────────────────────── */
.stButton>button {{
  border-radius:1px; border:1px solid var(--ink); background:var(--card); color:var(--ink);
  font-family:'Public Sans',sans-serif; font-size:11.5px; letter-spacing:.05em;
  text-transform:uppercase; font-weight:600; padding:.5rem 1.1rem;
}}
.stButton>button:hover {{ background:var(--ink); color:#fff; }}
.stButton>button[kind="primary"] {{ background:var(--ink); color:#fff; }}
.stButton>button[kind="primary"]:hover {{ background:var(--brassd); border-color:var(--brassd); }}
div[data-testid="stDataFrame"] {{ border:1px solid var(--rule); border-radius:0; }}
label, .stSelectbox label, .stRadio label, .stDateInput label, .stMultiSelect label, .stNumberInput label {{
  font-family:'Public Sans',sans-serif !important; font-size:10.5px !important;
  letter-spacing:.1em !important; text-transform:uppercase; color:var(--slate) !important;
}}
div[data-baseweb="select"]>div, div[data-baseweb="input"]>div {{ border-radius:1px; border-color:var(--rule); }}
hr {{ border-color:var(--rule); }}

@media (max-width:640px) {{
  .hl-row {{ flex-wrap:wrap; }} .hl {{ min-width:44%; border-bottom:1px solid var(--rule); }}
  .note-head .ghost {{ font-size:70px; top:-16px; }}
...
if st.session_state.auth is None:
    ...
    with mid:
        ...
        st.markdown(
            "<div style='border-top:3px double var(--rules);margin-top:14px;padding-top:14px;'>"
            "<div style='font-size:10.5px;letter-spacing:.14em;text-transform:uppercase;color:var(--brassd);"
            "font-weight:600;'>Trung tâm báo cáo vận hành</div>"
            "<h2 style='font-size:24px;font-weight:600;margin:4px 0 18px;'>Cổng truy cập báo cáo</h2></div>",
            unsafe_allow_html=True)
        if not USERS:
...
st.markdown(f"""<div class="letterhead">
  <div class="lockup"><div class="lockup-text">GHN · Báo cáo vận hành khu vực<span>Trung tâm điều hành chiến lược</span></div></div>
  <div class="period">Kỳ báo cáo <b>{REF:%d.%m.%Y}</b><br>Tổng hợp <b>{datetime.now():%H:%M}</b></div>
