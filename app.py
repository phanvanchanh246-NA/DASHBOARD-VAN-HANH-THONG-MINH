...
# ==========================================
# 1. CẤU HÌNH TRANG & GIAO DIỆN CHUNG (CSS - MẠNH MẼ, IN ĐẬM)
# ==========================================
st.set_page_config(page_title="Dashboard Vận Hành & Kinh Doanh", layout="wide", page_icon="📈")

st.markdown("""
    <style>
    /* Ép Font chữ toàn hệ thống mạnh mẽ hơn */
    html, body, [class*="css"]  {
    ...
    .banner {
        ...
        padding: 30px; border-radius: 12px; color: #ffffff;
        margin-bottom: 25px; display: flex; justify-content: space-between;
        align-items: center; border-bottom: 6px solid #f2c94c; 
    ...
    /* Khung cảnh báo AI */
    .ai-warning {
        background-color: #fff9e6; border-left: 6px solid #f2994a;
        padding: 20px; border-radius: 8px; margin-bottom: 20px;
        font-size: 16px; line-height: 1.8; color: #333333;
        box-shadow: 0px 4px 6px rgba(0,0,0,0.05);
    }
    .ai-warning b {
        color: #d35400; font-size: 18px; text-transform: uppercase;
    }
    
    /* Thẻ Tabs - Cứng cáp, đậm nét */
    .stTabs [data-baseweb="tab-list"] { gap: 12px; }
    .stTabs [data-baseweb="tab"] {
        font-weight: 800 !important; font-size: 17px !important; 
        border: 2px solid #203a43 !important;
        border-radius: 8px 8px 0px 0px !important; padding: 12px 28px !important;
        background-color: #f1f2f6 !important; color: #203a43 !important;
        text-transform: uppercase; letter-spacing: 0.5px;
    }
    .stTabs [aria-selected="true"] {
        background-color: #203a43 !important; color: #f2c94c !important;
        border: 2px solid #203a43 !important; 
...
# BẢNG MÀU BIỂU ĐỒ (TƯƠI MỚI & ĐẬM ĐÀ)
COLOR_BAR = '#1e90ff'        # Xanh dương sáng
COLOR_LINE_GTC = '#ff4757'   # Đỏ hồng ngọc
COLOR_LINE_RETURN = '#ff6348'# Đỏ tươi
COLOR_LINE_ODR = '#ffa502'   # Cam cháy
COLOR_LINE_PRICE = '#2ed573' # Xanh lá neon sáng

# ==========================================
# BẢO MẬT ĐĂNG NHẬP
# ==========================================
def check_login():
    st.markdown("<h2 style='text-align: center; color: #203a43; font-weight: 900;'>🔐 HỆ THỐNG QUẢN TRỊ NỘI BỘ GHN</h2>", unsafe_allow_html=True)
...
def styled_header(text, icon=""):
    st.markdown(f"""
        <div style="background-color: #2c3e50; color: #ffffff; padding: 14px 20px;
                    border-radius: 8px; border-left: 8px solid #f1c40f;
                    font-size: 22px; font-weight: 800; margin-top: 25px; margin-bottom: 20px;
...
def draw_combo_chart(df, x_col, bar_y, line_y, title, bar_name="Sản lượng", line_name="% GTC"):
    ...
    fig.add_trace(go.Bar(x=df[x_col], y=df[bar_y], name=bar_name, marker_color=COLOR_BAR, opacity=0.85), secondary_y=False)
    fig.add_trace(go.Scatter(x=df[x_col], y=df[line_y], name=line_name, mode='lines+markers', line=dict(color=COLOR_LINE_GTC, width=4), marker=dict(size=8, color='#ffffff', line=dict(color=COLOR_LINE_GTC, width=2))), secondary_y=True)
    fig.update_layout(title=dict(text=f"<b>{title}</b>", font=dict(size=20, color='#2f3542')), plot_bgcolor='#f1f2f6', paper_bgcolor='#ffffff', hovermode="x unified", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig.update_xaxes(showgrid=True, gridwidth=1, gridcolor='#dfe4ea')
    fig.update_yaxes(title_text=f"<b>{bar_name}</b>", secondary_y=False, showgrid=False)
    fig.update_yaxes(title_text=f"<b>{line_name}</b>", secondary_y=True, showgrid=True, gridcolor='#dfe4ea', range=[0, 100])
    return fig
...
def parse_vn_num(val):
    ...
    if val in ['nan', 'None', '', '0', '0.0']: return 0.0
    if ',' in val and '.' in val:
        ...
        parts = val.split('.')
        if len(parts) > 2: val = val.replace('.', '')
        else:
...
def clean_dataframe_numbers(df, text_cols):
    ...
    for col in df.columns:
        if col not in text_cols: df[col] = df[col].apply(parse_vn_num)
    return df
...
def get_real_business_data():
    ...
    try:
        ...
        kd_mapping = {'Doanh thu': 'Doanh Thu', 'Khách hàng liên hệ': 'Khách Liên Hệ', 'Khách hàng lên đơn': 'Khách Lên Đơn', 'Doanh thu KH mới': 'Doanh Thu KH Mới'}
...
def get_real_data():
    ...
    try:
        ...
        vh_mapping = {'%GTC': 'GTC', 'GTC (%)': 'GTC', 'Tỷ lệ GTC': 'GTC', '% GTC': 'GTC', 'Trả hàng': 'Trả Hàng', 'Tỷ lệ trả hàng': 'Trả Hàng', 'Volume_TTS': 'Volume TTS', 'GTC TTS': 'GTC_TTS', '%GTC_TTS': 'GTC_TTS', 'Ontime Giao TTS': 'ODR', 'ODR (%)': 'ODR'}
        df_vh = df_vh.rename(columns=vh_mapping)
        ns_mapping = {'GTC': '%GTC', 'Tỷ lệ GTC': '%GTC', '% GTC': '%GTC', 'Đơn giá': 'Đơn Giá', 'Số đơn': 'Số Đơn'}
...
st.markdown("""
    <div class="banner">
        <div>
            <h1 style="margin-bottom: 0;">DASHBOARD QUẢN LÝ VẬN HÀNH KINH DOANH GHN</h1>
            <p style="font-size: 16px; opacity: 0.9; font-weight: bold; margin-top: 5px;">Vận Hành - Năng Suất - KPI - Kinh Doanh</p>
        </div>
    </div>
""", unsafe_allow_html=True)
...
def render_ai_and_telegram(ai_result, tab_name, key_suffix):
    st.markdown(f'<div class="ai-warning"><b>🤖 CỐ VẤN AI ({tab_name.upper()}):</b><br><br>{ai_result}</div>', unsafe_allow_html=True)
    if st.button(f"📤 Bắn báo cáo {tab_name} lên nhóm Telegram", key=f"btn_tele_{key_suffix}"):
        if not TELEGRAM_TOKEN or TELEGRAM_TOKEN == "ĐIỀN_TOKEN_BOT_TELEGRAM_VÀO_ĐÂY":
            ...
            try:
                ...
                if req.status_code == 200: st.success("✅ Đã bắn báo cáo thành công lên nhóm Telegram!")
                else: st.error(f"❌ Lỗi khi gửi Telegram (Mã lỗi {req.status_code})")
...
with tab1:
    ...
    if len(date_range_vh) == 2: mask_vh &= (df_vanhanh['Ngày'] >= pd.to_datetime(date_range_vh[0])) & (df_vanhanh['Ngày'] <= pd.to_datetime(date_range_vh[1]))
    if buu_cuc_vh != "Tất cả": mask_vh &= (df_vanhanh['Bưu Cục'] == buu_cuc_vh)
    ...
    with chart_col2:
        fig_return = px.line(df_trend_display, x='Ngày', y='Trả Hàng', markers=True, title=f"<b>Tỷ lệ Trả Hàng ({view_mode_vh}) (%)</b>")
        fig_return.update_traces(line=dict(color=COLOR_LINE_RETURN, width=4), marker=dict(size=10, color='#ffffff', line=dict(color=COLOR_LINE_RETURN, width=2)))
        fig_return.update_layout(plot_bgcolor='#f1f2f6', paper_bgcolor='#ffffff', hovermode="x unified", title_font=dict(size=20, color='#2f3542'))
    ...
    with chart_col4:
        fig_odr = px.line(df_tts, x='Ngày', y='ODR', markers=True, title="<b>Tỷ lệ Ontime TTS (ODR TikTokShop)</b>")
        fig_odr.update_traces(line=dict(color=COLOR_LINE_ODR, width=4), marker=dict(size=10, color='#ffffff', line=dict(color=COLOR_LINE_ODR, width=2)))
        fig_odr.update_layout(plot_bgcolor='#f1f2f6', paper_bgcolor='#ffffff', yaxis=dict(range=[70, 100]), hovermode="x unified", title_font=dict(size=20, color='#2f3542'))
    ...
    colors_ca = ['#1e90ff', '#2ed573', '#ff4757', '#ffa502']
    for idx, ca_name in enumerate(df_ca['Ca'].unique()):
        df_ca_sub = df_ca[df_ca['Ca'] == ca_name]
        color_idx = idx % len(colors_ca)
        fig_ca.add_trace(go.Bar(x=df_ca_sub['TrụcX'], y=df_ca_sub['Volume'], name=f"Volume {ca_name}", marker_color=colors_ca[color_idx], opacity=0.85), secondary_y=False)
        fig_ca.add_trace(go.Scatter(x=df_ca_sub['TrụcX'], y=df_ca_sub['GTC'], name=f"%GTC {ca_name}", mode='lines+markers', line=dict(color='#2f3542', width=3), marker=dict(size=8, color=colors_ca[color_idx])), secondary_y=True)

    fig_ca.update_layout(title=dict(text="<b>Sản Lượng và Tỷ Lệ GTC Theo Ca Làm Việc</b>", font=dict(size=20, color='#2f3542')), plot_bgcolor='#f1f2f6', hovermode="x unified", barmode='group')
    fig_ca.update_yaxes(title_text="<b>Sản lượng</b>", secondary_y=False)
    fig_ca.update_yaxes(title_text="<b>% GTC</b>", secondary_y=True, range=[0, 100])
    ...
    if st.button("🔍 Nhờ AI Phân tích Vận Hành", type="primary", key="btn_ai_vh"):
        with st.spinner("🔄 AI đang phân tích dữ liệu Vận Hành..."):
            prompt_vh = f"Dữ liệu Vận Hành: Tổng đơn: {df_vh_filtered['Volume'].sum()}, GTC: {df_vh_filtered['GTC'].mean():.2f}%, ODR: {df_vh_filtered['ODR'].mean():.2f}%. Hãy phân tích."
...
with tab2:
    ...
    with f_col1: date_range_ns = st.date_input("Khoảng thời gian (Nhân sự)", [df_nhansu['Ngày'].min(), df_nhansu['Ngày'].max()], key="date_ns")
    with f_col2: loai_hang_filter = st.multiselect("Lọc Loại Hàng", ["Hàng nhỏ", "Hàng cồng kềnh", "FULL"], default=["Hàng nhỏ", "Hàng cồng kềnh", "FULL"], key="lh_filter")
    with f_col3: buu_cuc_ns = st.selectbox("Lọc Bưu cục", ["Tất cả"] + list(df_nhansu['Bưu Cục'].unique()), key="bc_ns_tab2")
    with f_col4:
        if buu_cuc_ns == "Tất cả": nv_list = ["Tất cả"] + list(df_nhansu['Nhân Viên'].unique())
        else: nv_list = ["Tất cả"] + list(df_nhansu[df_nhansu['Bưu Cục'] == buu_cuc_ns]['Nhân Viên'].unique())
        nhan_vien_ns = st.selectbox("Lọc Nhân viên", nv_list, key="nv_ns_tab2")

    mask_ns = pd.Series(True, index=df_nhansu.index)
    if len(date_range_ns) == 2: mask_ns &= (df_nhansu['Ngày'] >= pd.to_datetime(date_range_ns[0])) & (df_nhansu['Ngày'] <= pd.to_datetime(date_range_ns[1]))
    if loai_hang_filter: mask_ns &= df_nhansu['Loại Hàng'].isin(loai_hang_filter)
    if buu_cuc_ns != "Tất cả": mask_ns &= (df_nhansu['Bưu Cục'] == buu_cuc_ns)
    if nhan_vien_ns != "Tất cả": mask_ns &= (df_nhansu['Nhân Viên'] == nhan_vien_ns)
    ...
    with chart_ns1:
        if nhan_vien_ns != "Tất cả": title_dongia = f"Biến động Đơn Giá của {nhan_vien_ns}"
        elif buu_cuc_ns != "Tất cả": title_dongia = f"Đơn giá trung bình tại {buu_cuc_ns}"
        else: title_dongia = "Đơn giá trung bình toàn hệ thống"
        df_dongia = df_ns_filtered.groupby('Ngày')['Đơn Giá'].mean().reset_index()
        fig_dg = px.line(df_dongia, x='Ngày', y='Đơn Giá', markers=True, title=f"<b>{title_dongia}</b>")
        fig_dg.update_traces(line=dict(color=COLOR_LINE_PRICE, width=4), marker=dict(size=10, color='#ffffff', line=dict(color=COLOR_LINE_PRICE, width=2)))
        fig_dg.update_layout(plot_bgcolor='#f1f2f6', hovermode="x unified", title_font=dict(size=20, color='#2f3542'), yaxis_title="<b>VNĐ</b>")
    ...
    if st.button("🔍 Nhờ AI Phân tích Nhân sự & Chi phí", type="primary", key="btn_ai_ns"):
        with st.spinner("🔄 AI đang phân tích dữ liệu Năng Suất..."):
            st.session_state.ai_ns_result = get_ai_analysis("Phân tích năng suất và chi phí dựa trên dữ liệu đang xem.")
...
with tab3:
    ...
    with st.expander("⚙️ ĐIỀU CHỈNH KPI (Sẽ tự động lưu lại theo từng Khu vực/Bưu cục)", expanded=True):
        ...
        with kpi_col1: st.session_state.kpi_gtc_dict[target_bc_kpi] = st.number_input(f"Mục tiêu KPI %GTC ({target_bc_kpi})", min_value=0.0, max_value=100.0, value=float(st.session_state.kpi_gtc_dict[target_bc_kpi]), step=0.5)
        with kpi_col2: st.session_state.kpi_tts_dict[target_bc_kpi] = st.number_input(f"Mục tiêu KPI %GTC TikTokShop ({target_bc_kpi})", min_value=0.0, max_value=100.0, value=float(st.session_state.kpi_tts_dict[target_bc_kpi]), step=0.5)
        with kpi_col3: st.session_state.kpi_odr_dict[target_bc_kpi] = st.number_input(f"KPI Ontime Giao TTS (ODR) ({target_bc_kpi})", min_value=0.0, max_value=100.0, value=float(st.session_state.kpi_odr_dict[target_bc_kpi]), step=0.5)

    t3_col1, t3_col2 = st.columns(2)
    with t3_col1: date_range_kpi = st.date_input("Chọn thời gian", [df_vanhanh['Ngày'].min(), df_vanhanh['Ngày'].max()], key="date_kpi")
    with t3_col2: buu_cuc_kpi = st.selectbox("Chọn Bưu cục để XEM số liệu", ["Tất cả"] + list(df_vanhanh['Bưu Cục'].unique()), key="bc_kpi")

    mask_kpi = pd.Series(True, index=df_vanhanh.index)
    if len(date_range_kpi) == 2: mask_kpi &= (df_vanhanh['Ngày'] >= pd.to_datetime(date_range_kpi[0])) & (df_vanhanh['Ngày'] <= pd.to_datetime(date_range_kpi[1]))
    if buu_cuc_kpi != "Tất cả": mask_kpi &= (df_vanhanh['Bưu Cục'] == buu_cuc_kpi)
    ...
    def create_gauge(title, value, target, inverse_color=False):
        color = "#2ed573" if (value >= target and not inverse_color) or (value <= target and inverse_color) else "#ff4757"
        fig = go.Figure(go.Indicator(
            mode = "gauge+number+delta", value = value, domain = {'x': [0, 1], 'y': [0, 1]},
            title = {'text': f"<b>{title}</b>", 'font': {'size': 18, 'color':'#2f3542'}},
            delta = {'reference': target, 'increasing': {'color': "#ff4757" if inverse_color else "#2ed573"}, 'decreasing': {'color': "#2ed573" if inverse_color else "#ff4757"}},
            gauge = {
                'axis': {'range': [None, 100], 'tickwidth': 2, 'tickcolor': "#2f3542"},
                'bar': {'color': color},
                'steps': [{'range': [0, 100], 'color': "#f1f2f6"}],
    ...
    with gauge_col1: st.plotly_chart(create_gauge("Tỷ lệ GTC Chung (%)", actual_gtc, current_kpi_gtc), use_container_width=True)
    with gauge_col2: st.plotly_chart(create_gauge("Tỷ lệ GTC TikTokShop (%)", actual_tts, current_kpi_tts), use_container_width=True)
    with gauge_col3: st.plotly_chart(create_gauge("Ontime Giao TTS ODR (%)", actual_odr, current_kpi_odr, inverse_color=True), use_container_width=True)
    ...
    if st.button("🔍 AI Đánh giá mức độ đạt KPI", type="primary", key="btn_ai_kpi"):
        with st.spinner("🔄 AI đang đối chiếu số liệu với mục tiêu KPI..."):
            st.session_state.ai_kpi_result = get_ai_analysis("Đánh giá KPI hiện tại và đưa ra cảnh báo.")
...
with tab4:
    ...
    with t4_col1: date_range_kd = st.date_input("Chọn thời gian", [df_kinhdoanh['Ngày'].max() - timedelta(days=7), df_kinhdoanh['Ngày'].max()], key="date_kd")
    with t4_col2: buu_cuc_kd = st.selectbox("Chọn Bưu cục để XEM số liệu", ["Tất cả"] + list(df_kinhdoanh['Bưu Cục'].unique()), key="bc_kd")
    with t4_col3: view_type = st.selectbox("Góc nhìn báo cáo", ["Theo Ngày", "Theo Tuần", "Theo Tháng"], key="view_kd")

    current_date = df_kinhdoanh['Ngày'].max()
    if len(date_range_kd) == 2: current_date = pd.to_datetime(date_range_kd[1])
        
    mask_kd = pd.Series(True, index=df_kinhdoanh.index)
    if buu_cuc_kd != "Tất cả": mask_kd &= (df_kinhdoanh['Bưu Cục'] == buu_cuc_kd)
    ...
    if view_type == "Theo Ngày":
        ...
        rev_n = df_filtered_kd[(df_filtered_kd['Ngày'] >= start_w) & (df_filtered_kd['Ngày'] <= start_w + timedelta(days=6))]['Doanh Thu'].sum()
        rev_prev = df_filtered_kd[(df_filtered_kd['Ngày'] >= start_w - timedelta(days=7)) & (df_filtered_kd['Ngày'] <= start_w - timedelta(days=1))]['Doanh Thu'].sum()
        ...
        rev_prev = df_filtered_kd[(df_filtered_kd['Ngày'] >= start_m_prev) & (df_filtered_kd['Ngày'] <= start_m - timedelta(days=1))]['Doanh Thu'].sum()
        label_prev = "So với M-1 (Tháng trước)"

    kpi_dt_val = st.session_state.kpi_dt_dict.get(buu_cuc_kd, 30000000.0)
    if view_type == "Theo Tuần": kpi_dt_val *= 7
    if view_type == "Theo Tháng": kpi_dt_val *= 30
    ...
    if len(date_range_kd) == 2: mask_kd_range &= (df_kinhdoanh['Ngày'] >= pd.to_datetime(date_range_kd[0])) & (df_kinhdoanh['Ngày'] <= pd.to_datetime(date_range_kd[1]))
    
    df_plot_kd_display = df_kinhdoanh[mask_kd_range].copy()
    if view_type == "Theo Tuần": df_plot_kd_display['Ngày'] = df_plot_kd_display['Ngày'].dt.to_period('W').apply(lambda r: r.start_time)
    elif view_type == "Theo Tháng": df_plot_kd_display['Ngày'] = df_plot_kd_display['Ngày'].dt.to_period('M').apply(lambda r: r.start_time)
        
    df_plot_kd_grouped = df_plot_kd_display.groupby('Ngày').agg({'Doanh Thu': 'sum', 'Khách Liên Hệ': 'sum', 'Khách Lên Đơn': 'sum', 'Doanh Thu KH Mới': 'sum'}).reset_index()

    chart_kd1, chart_kd2 = st.columns(2)
    with chart_kd1:
        fig_rev = px.bar(df_plot_kd_grouped, x='Ngày', y='Doanh Thu', title=f"<b>Biểu đồ Doanh Thu & KPI ({buu_cuc_kd})</b>", color_discrete_sequence=[COLOR_BAR])
        fig_rev.add_hline(y=kpi_dt_val, line_dash="dash", line_color=COLOR_LINE_RETURN, annotation_text="KPI Mục Tiêu")
        fig_rev.update_layout(plot_bgcolor='#f1f2f6', title_font=dict(size=20, color='#2f3542'))
        st.plotly_chart(fig_rev, use_container_width=True)

    with chart_kd2:
        fig_funnel = go.Figure(go.Funnel(y=["Khách Liên Hệ", "Khách Lên Đơn (Chuyển đổi)"], x=[df_plot_kd_grouped['Khách Liên Hệ'].sum(), df_plot_kd_grouped['Khách Lên Đơn'].sum()], textinfo="value+percent initial", marker={"color": [COLOR_BAR, COLOR_LINE_PRICE]}))
        fig_funnel.update_layout(title=dict(text=f"<b>Phễu chuyển đổi KH Mới ({view_type})</b>", font=dict(size=20, color='#2f3542')))
    ...
    if st.button("🔍 AI Cố vấn Kinh Doanh & Sales", type="primary", key="btn_ai_kd"):
        with st.spinner("🔄 AI đang phân tích hiệu suất Kinh Doanh..."):
            st.session_state.ai_kd_result = get_ai_analysis("Phân tích dữ liệu kinh doanh hiện tại và đưa ra chiến lược sales.")
